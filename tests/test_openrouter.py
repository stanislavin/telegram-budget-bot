...
def mock_openrouter_response():
    ...
    with patch('requests.post') as mock_post:
        ...
        mock_response.json.return_value = {
            'choices': [{'message': {'content': '100.00,USD,Food,Groceries'}}]
        }
...
async def test_process_with_openrouter_success(mock_openrouter_response):
    ...
    message = "100 usd food groceries"
    result, error = await process_with_openrouter(message)
    
    assert error is None
    assert result == (100.0, 'USD', 'Food', 'Groceries')
...
async def test_process_with_openrouter_parsing_error():
    ...
    with patch('requests.post') as mock_post:
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            'choices': [{'message': {'content': '100.00,USD,Food'}}]
        } # Missing description
        mock_post.return_value = mock_response
        
        message = "100 usd food"
        result, error = await process_with_openrouter(message)
